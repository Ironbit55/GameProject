...
Quaternion Quaternion::FromMatrix(const Matrix4 &m)	{
	...
	q.w = sqrt(nclgl_max(0.0f, (1.0f + m.values[0] + m.values[5] + m.values[10]))) / 2.0f;
	q.x = sqrt(nclgl_max(0.0f, (1.0f + m.values[0] - m.values[5] - m.values[10]))) / 2.0f;
	q.y = sqrt(nclgl_max(0.0f, (1.0f - m.values[0] + m.values[5] - m.values[10]))) / 2.0f;
	q.z = sqrt(nclgl_max(0.0f, (1.0f - m.values[0] - m.values[5] + m.values[10]))) / 2.0f;
